...
1) РАЗЛОЖИ HARNESS на диск из ~/workshops/aiweekend/workshop.md (скачан в шаге 4). Ты ничего
   ...
   - каждая пара «строка cat > …harness/… <<'HARNESS_EOF_Z9'» → следующая строка ровно
     ...
     бэктики Python/Go НЕ обрывают). Всего 27 файлов: §10 (skills/expert-panel.md), §11
...
Когда всё готово — напиши «БУТСТРАП ГОТОВ», повтори сводку версий (шаг 5) и добавь счётчики harness:
   echo "$(ls ~/workshops/aiweekend/harness/agents/*.md 2>/dev/null|wc -l) правил / $(ls ~/workshops/aiweekend/harness/skills/*.md 2>/dev/null|wc -l) скилов / $(ls ~/workshops/aiweekend/harness/*.yaml 2>/dev/null|wc -l) порогов / $(ls ~/workshops/aiweekend/harness/evals/ 2>/dev/null|wc -l) evals"  # ждём 11 / 9 / 2 / 5
WSP_EOF
...
Рабочий каталог на весь воркшоп — `~/workshops/aiweekend` (в нём уже открыт TUI из шага 7; сам шаг 8 идёт готовой командой из второго окна терминала). Этот промт — **фоновая работа**: отдай его агенту и возвращайся к лекции; агент **одним прогоном** ставит тулчейн И раскладывает 24 harness-файла на диск, а ты вернёшься к чекпоинту, когда он отчитается.
...
> **Чекпоинт бутстрапа.** Агент написал «БУТСТРАП ГОТОВ» и отдал ДВЕ сводки: (1) девять версий — Python (`uv run python -V`), Go, Docker, VS Code (`code --version`), `docker info` без ошибок; (2) счётчики harness `11 / 9 / 2 / 5`. Сверь harness сам:
>
> ```bash
> echo "$(ls ~/workshops/aiweekend/harness/agents/*.md 2>/dev/null|wc -l) / $(ls ~/workshops/aiweekend/harness/skills/*.md 2>/dev/null|wc -l) / $(ls ~/workshops/aiweekend/harness/*.yaml 2>/dev/null|wc -l) / $(ls ~/workshops/aiweekend/harness/evals/ 2>/dev/null|wc -l)"   # 11 / 9 / 2 / 5
> ```
>
...
&gt; **Счётчики не сошлись (`11 / 9 / 2 / 5`)?** Вернись к Шагу 8 — агент найдёт недостающий `cat`-блок в `workshop.md` и выполнит заново (идемпотентно). Вставлять heredoc вручную не требуется.
...
**Чекпоинт:** прогони `cd ~/workshops/aiweekend/qrsvc-py && uv run pytest -q --cov=.` сам; найди в логе агента красный прогон до реализации; `grep -r "@given" tests/` находит property-тест; `test -f harness/skills/feature.md && echo HARNESS_OK` подтверждает, что project-local harness не тронут.
...
Подготовь инструменты гейта и сними снимок рабочего кода в индекс (чтобы откатить инжект):
...
**«Сломай» — единственное доказательство, что тесты живые.** Зелёный набор ничего не стоит, пока не убедишься, что он умеет краснеть. Инжектируй семантический баг в подбор версии QR: код начинает считать вместимость всегда как для EC=`"M"`, игнорируя запрошенный `L/Q/H`. Unit/property/e2e-тесты на границы вместимости обязаны покраснеть:

Найди функцию подбора версии и обращения к таблице вместимости:

```bash
cd ~/workshops/aiweekend/qrsvc-py
grep -rn "error_correction" ~/workshops/aiweekend/qrsvc-py --include='*.py'
```

✋ [ручной шаг] В `src/qrsvc/qr.py` найди строку вида `QR_CAPACITY[version][error_correction]` и временно замени индекс `error_correction` на `"M"`: получится `QR_CAPACITY[version]["M"]`. `M` здесь — один из четырёх уровней коррекции ошибок QR (`L/M/Q/H`, в спеке они перечислены в `enum`). Мы специально ломаем правило: если клиент запросил, например, `"H"` или `"L"`, сервис начнёт считать вместимость по уровню `"M"`, и тесты на границы обязаны покраснеть.
...
**Ожидаем:** после инжекта — КРАСНЫЙ (обычно падают unit/property/e2e на границах `L/H`); после отката — зелёный. Не покраснел — тест пустышка (`assert True`-класс): чини тест, а не код.

💡 То же на автомате делает mutation-гейт: `uv run mutmut run` вносит десятки таких инжектов и показывает выживших — тесты, которые их не поймали. Ручной инжект — чтобы увидеть механику своими глазами.
...
cd ~/workshops/aiweekend
opencode run --model devhands/z-ai/glm-5.2 \
  "ты в ~/workshops/aiweekend. Сделай ревью проекта qrsvc-py/ строго по qrsvc-py/harness/skills/review.md; qrsvc-py/harness/** и общий harness/** read-only, код НЕ правь. SPEC_FILE — qrsvc-py/api/openapi.yaml; вердикт «мержить / не мержить» + находки с привязкой к П-пунктам" < /dev/null > /tmp/review_flash.txt &
opencode run --model devhands/deepseek/deepseek-v4-flash \
  "ты в ~/workshops/aiweekend. Сделай ревью проекта qrsvc-py/ строго по qrsvc-py/harness/skills/review.md; qrsvc-py/harness/** и общий harness/** read-only, код НЕ правь. SPEC_FILE — qrsvc-py/api/openapi.yaml; вердикт «мержить / не мержить» + находки с привязкой к П-пунктам" < /dev/null > /tmp/review_free.txt &
wait
...
**Гейт на коммит + первый тег вклада.** Создай гейт и хук прямо в `qrsvc-py` (свой `Makefile` с целью `gate` — это НЕ harness/evals, а гейт твоего проекта):
...
gate:
	uv run ruff check .
	uv run pytest -q --cov=. --cov-fail-under=0
MAKE_EOF
...
cd ~/workshops/aiweekend
opencode run --model devhands/z-ai/glm-5.2 \
  "прогони make gate на сгенерированном harness под $L2; компиляция/линт/типы красные — почини до зелёного; инварианты, пороги, паритет и существующие тесты не трогай"
```

Это та же eval-петля harness, применённая к самому harness (см. §11.1): зелёный `make gate` — деплой разрешён, красный валит деплой на первом же нарушении.


💡 **Что здесь важно (ключевые точки вызова словами):**
- **Оба скила рядом — ядро паритета.** Паритет вычислим только когда судья видит оба языка против одной спеки; отсюда в промте «положи `resilience-$L1.md` рядом». С одним артефактом судья проверит полноту, но не паритет.
...
- **Компайл-нити добивает сам gate.** Совет закрывает инварианты и паритет по смыслу; синтаксическую валидность добивает сгенерированный `make gate` + fix-loop — тот же рекурсивный приём, что в 11.1.
...
( cd my-json-go && opencode run --model devhands/z-ai/glm-5.2 "$(sed 's/Реализуй на <Go | Python>/Реализуй на Go/' /tmp/json_service_prompt.txt)" )
( cd my-json-py && opencode run --model devhands/z-ai/glm-5.2 "$(sed 's/Реализуй на <Go | Python>/Реализуй на Python/' /tmp/json_service_prompt.txt)" )
```
...
( cd my-qr-go && opencode run --model devhands/z-ai/glm-5.2 "$(sed 's/Реализуй на <Go | Python>/Реализуй на Go/' /tmp/qr_service_prompt.txt)" )
( cd my-qr-py && opencode run --model devhands/z-ai/glm-5.2 "$(sed 's/Реализуй на <Go | Python>/Реализуй на Python/' /tmp/qr_service_prompt.txt)" )
```
...
Единый помощник (поднять → дождаться healthz). Аргументы: `$1` — image, `$2` — подпись, `$3` — команда-override для Python; для Go пусто.
...
bench () {
  ...
  if [ -n "$3" ]; then
    docker run -d --name svc --cpus=2 --memory=512m -p ${PORT}:8080 -e GOMAXPROCS=2 "$1" sh -c "$3" >/dev/null
  else
    docker run -d --name svc --cpus=2 --memory=512m -p ${PORT}:8080 -e GOMAXPROCS=2 "$1" >/dev/null
  fi
  echo ">>> $2"; for i in $(seq 1 60); do curl -sf http://localhost:${PORT}/healthz >/dev/null && break; sleep 0.5; [ "$i" = 60 ] && { echo "healthz не поднялся за 30с"; docker logs svc; return 1; }; done
}
```
...
После каждого замера задай значения из oha-TUI и допиши строку:

```bash
export svc=json-go mode=baseline rps=0 p50=0 p99=0 non2xx=0
echo "$svc,$mode,$rps,$p50,$p99,$non2xx" >> scoreboard.csv
```
...
> 🖐 **Путь руками** — видишь каждый замер глазами; отдать весь раунд агенту — §20.4.6.

Стенд уже собран в §20.4.1–20.4.2 спеки (heredoc'и `json-go`/`json-py` + `Dockerfile`). Здесь — только прогон. Шаги ниже — руками, чтобы видеть механику; хочешь отдать весь прогон агенту — промт-автопрогон в §20.4.6. Хелпер `bench` (§20.4.5) поднимает сервис и ждёт `/healthz`:

Preflight до прогона: образы собраны, `oha` на месте, порт `8080` свободен. Хелпер из §20.4.5 запускает контейнер с `--cpus=2 --memory=512m -e GOMAXPROCS=2` и ждёт `/healthz`.

```bash
cd ~/workshops/aiweekend/lang-battle
docker image inspect json-go json-py qr-go qr-py >/dev/null 2>&1 && echo "образы: ok" || echo "СТОП: нет образа, собери из §20.4.1–20.4.4 (docker build -t json-go ...)"
...
#### 22.1.2 🎬 AI-команда ролей (dev · tester · reviewer) + совет-техлид: два цикла обратной связи (проверено вживую)

**Что вы унесёте.** Прогоняемую промтами AI-команду из трёх агентов — **dev / tester / reviewer**, поднятых как ОТДЕЛЬНЫЕ агенты `opencode run` (каждый по своему дев-скилу, обмен через `.handoff/*.md`), с **двумя вложенными циклами обратной связи**: внутренним (reviewer заворачивает PR к dev) и внешним (совет как техлид принимает или возвращает готовый PR). Одна мысль курса: **агенты ПРОИЗВОДЯТ (ось-роль), совет СУДИТ (ось-модель)** — это две разные оси, не приём дважды.

**Агенты (производят) vs совет (судит):**

| | Агенты (конвейер) | Совет (приёмка) |
|---|---|---|
| Что делает | **ПРОИЗВОДИТ** артефакт | **СУДИТ** артефакт |
| Ось разнообразия | роли/скилы (dev · tester · reviewer) | модели/семейства (3 судьи) |
| Топология | последовательная цепочка, handoff | параллельные судьи → majority |
| Различаются | **РАБОТОЙ** (у каждого своя) | **МНЕНИЕМ** (у каждого тот же вопрос) |

> Агенты можно поднять на ОДНОЙ модели (разводит скил), совет — обязательно на РАЗНЫХ семействах (иначе три одинаковых мнения). Спутать оси = либо просить конвейер «проголосовать», либо просить совет «дописать фичу».

**Никакой новой машинерии:** `.handoff/*.md` — из слоя 1 (21.1.1), цикл судей — из защиты харнеса, скил суда — `harness/skills/expert-panel.md`. Новое одно: роли конвейера подняты как ОТДЕЛЬНЫЕ агенты-процессы, каждый со свежим контекстом.

**Картинка целиком — конвейер и две петли обратной связи:**

```mermaid
flowchart LR
    DEV["👨‍💻 dev<br/>пишет"] --> TEST["🧪 tester<br/>тесты"]
    TEST --> REV["🔍 reviewer<br/>ревью"]
    REV -.->|"не мержить · находки"| DEV
    REV ==>|"мержить"| CO["⚖️ СОВЕТ · техлид<br/>3 модели, majority"]
    CO -.->|"FAIL · на доработку"| DEV
    CO ==>|"PASS"| DONE["✅ в репозиторий"]
```

💡 **Как читать:** сплошные стрелки `==>` — производство (конвейер вперёд), пунктирные `-.->` — **доработка назад к dev**. Две петли: **внутренняя** (reviewer заворачивает — внутрикомандное ревью, ДО того как PR уйдёт наверх) и **внешняя** (совет-техлид заворачивает уже готовый PR). dev и производит, и дорабатывает; reviewer и совет — заворачивают, но код не пишут.

🤖 **Шаг 0** — каталог handoff и задача. При желании поменяй строку `TASK`: пример ниже добавляет необязательный параметр `label`.

```bash
cd ~/workshops/aiweekend && mkdir -p .handoff
export TASK="добавь в POST /api/v1/qr необязательный параметр label: string, maxLength 64; в ответе label present ⟺ задан и длиной ≤64, иначе 422"
```

**① Агенты производят: dev → tester → reviewer.** Три отдельных `opencode run`, свежий контекст у каждого, обмен через `.handoff/`. Read-only reviewer и разные процессы снимают self-preference **на уровне процессов, а не уговоров в одном промте**.

> **Ключевое: каждый агент работает по СОВЕТ-генеренному скилу, не по заглушке.** `harness/skills/{feature,tests,review}.md` — это те самые **энтерпрайзные** плейбуки, что совет собрал под твой язык в частях 2–3 из универсальных инвариантов (resilience, security, contracts, testing…), уровня твоего эталона. Качество работы команды = качество этих скилов: dev пишет по энтерпрайз-`feature.md`, tester ужесточает по энтерпрайз-`tests.md`, reviewer заворачивает по энтерпрайз-`review.md`. Слабый скил → слабая команда; поэтому скилы и порождены советом, а не написаны на коленке.

```bash
cd ~/workshops/aiweekend
opencode run --model devhands/z-ai/glm-5.2 \
  "ты агент DEV. Ты в ~/workshops/aiweekend (qrsvc-py/ и harness/ рядом). Реализуй в qrsvc-py/ СТРОГО по harness/skills/feature.md (контракт-первым: правка qrsvc-py/api/openapi.yaml → ПАДАЮЩИЙ contract-тест → код): ${TASK}. Картинку не генерируй. Итог и список файлов → .handoff/01-dev.md; временное держи внутри qrsvc-py."
opencode run --model devhands/z-ai/glm-5.2 \
  "ты агент TESTER, свежий контекст. Прочитай .handoff/01-dev.md и изменённые файлы qrsvc-py/. Ужесточи тесты по harness/skills/tests.md: property на нерушимое правило + границы (64 ок / 65→422 / пустой / отсутствует) + для каждого теста назови инжект-баг, что он ловит. Код фичи НЕ трогай. Итог → .handoff/02-tester.md."
opencode run --model devhands/z-ai/glm-5.2 \
  "ты агент REVIEWER, READ-ONLY, свежий контекст. Прочитай .handoff/01-dev.md, .handoff/02-tester.md и код qrsvc-py/. Отревьюй СТРОГО по harness/skills/review.md; SPEC_FILE=qrsvc-py/api/openapi.yaml; ищи реальные дыры (нет max_length/валидации границ, edge, malformed, image_url без энкодинга). Код НЕ правь. Вердикт «мержить/не мержить» + нумерованные находки с привязкой к строкам → .handoff/03-review.md."
```

**② Внутренний луп dev↔reviewer (внутрикомандное ревью).** Ревьюер вынес «не мержить» — команда итерируется САМА, до того как PR уйдёт наверх. Гоняешь, пока reviewer не скажет «мержить» (лимит 2–3 витка):

✋ Сначала прочти `.handoff/03-review.md`. Если там «не мержить», запускай пару dev→reviewer ниже и повторяй её до «мержить».

```bash
cd ~/workshops/aiweekend
opencode run --model devhands/z-ai/glm-5.2 \
  "ты агент DEV. Ревьюер завернул PR. Прочитай его находки в .handoff/03-review.md и почини qrsvc-py/ строго по ним; тесты-эталон не ослабляй, только добавляй. Что починил → .handoff/04-dev-fix.md."
opencode run --model devhands/z-ai/glm-5.2 \
  "ты агент REVIEWER, READ-ONLY. Прочитай .handoff/04-dev-fix.md и код. Закрыты ли ТВОИ прошлые находки? Вердикт «мержить/не мержить» + что осталось → .handoff/05-review-r2.md."
```

💡 **Что это доказывает:** reviewer — внутренний гейт команды: **«сам написал — сам одобрил» невозможно**, потому что это другой агент-процесс, и он заворачивает работу назад к dev. Только когда команда сама довольна — PR идёт на приёмку совету.

🎬 **Реальный прогон (проверено вживую).** 2 витка: dev собрал → **reviewer завернул (НЕ МЕРЖИТЬ)**, поймав `maxLength` в СИМВОЛАХ вместо байтов (`"я"×2953` = 5906 байт → ложный 200 вместо 422), `image_url` через `quote()` без экранирования `/` (path-injection), wrong-type → 422 вместо 400. dev починил строго по находкам (байтовая валидация, `quote(safe="")`, 400 на malformed) → тесты **28→37 зелёные** → **reviewer: МЕРЖИТЬ**. Внутренняя петля закрылась за 2 витка — ДО того, как PR ушёл совету.

**③ Совет судит готовый PR — и в живом прогоне ЗАВЕРНУЛ его (проверено вживую).** Три модели-семейства параллельно по `expert-panel.md`, вердикты в файлы, majority:

```bash
cd ~/workshops/aiweekend
cat > /tmp/team_verdict.txt <<'TV_EOF'
Ты судья экспертного совета, действуй по harness/skills/expert-panel.md. Артефакт на приёмку —
результат AI-команды: изменения qrsvc-py/ + цепочка .handoff/*.md. НЕ доверяй ревьюеру слепо —
пересуди сам. Оцени по 3 критериям, строка на каждый: VERDICT=<критерий>:<PASS|FAIL|INCONCLUSIVE> — обоснование:
1) Контракт-паритет со спекой qrsvc-py/api/openapi.yaml; 2) Тесты-живые (реально ловят нарушение, не assert-True); 3) Ревью-закрыто. Затем ОБЩИЙ ВЕРДИКТ.
TV_EOF
for M in deepseek/deepseek-v4-pro z-ai/glm-5.2 moonshotai/kimi-k2.7-code; do
  opencode run --model "devhands/$M" "$(cat /tmp/team_verdict.txt)" < /dev/null > "team_verdict_${M##*/}.txt" &   # < /dev/null: иначе фоновый opencode зависает на init без stdin
done; wait; ls team_verdict_*.txt
```

🎬 **Реальный прогон (проверено вживую).** Ревьюер сказал **МЕРЖИТЬ** — а совет **единогласно завернул (FAIL 3/3)**, поймав дефект, который ревьюер пропустил: `image_url` строился без URL-кодирования (`urllib.parse.quote`), значение `data` со спецсимволами `& = ? #` даёт инъекцию query-параметров.

```text
== СОВЕТ СУДИТ (majority) ==            deepseek  glm   kimi  большинство
Контракт-паритет                        PASS      FAIL  PASS  PASS 2:1
Тесты-живые                             PASS      PASS  PASS  PASS 3:0
Resilience / безопасность               FAIL      FAIL  FAIL  FAIL 3:0   ← image_url без quote
ОБЩИЙ ВЕРДИКТ                                                  FAIL 3:0 (вернуть на доработку)
```

**④ Техлид-цикл: совет вернул → dev дорабатывает → совет пересуживает (проверено вживую).** Внешний луп. Совет — как техлид: не переписывает код команды, а возвращает с конкретными находками:

✋ Собери majority-находки из `team_verdict_*.txt`. Если хочешь другой набор находок, поменяй `COUNCIL_FINDINGS` в первой строке.

```bash
cd ~/workshops/aiweekend
export COUNCIL_FINDINGS="image_url строится без urllib.parse.quote → инъекция query-параметров; нет max_length"
opencode run --model devhands/deepseek/deepseek-v4-pro \
  "ты агент DEV. Совет-техлид вернул PR на доработку с находками: ${COUNCIL_FINDINGS}. Почини qrsvc-py/ СТРОГО по находкам (заэнкодь data через urllib.parse.quote, добавь max_length и тест на спецсимволы/Unicode); эталонные тесты не ослабляй. Что починил → .handoff/06-dev-fix.md."
for M in deepseek/deepseek-v4-pro z-ai/glm-5.2 moonshotai/kimi-k2.7-code; do
  opencode run --model "devhands/$M" "$(cat /tmp/team_verdict.txt)" < /dev/null > "team_verdict2_${M##*/}.txt" &
done; wait
```

🎬 **Реальная доработка dev и пересуд (проверено вживую):**

```diff
- data: str = Field(..., min_length=1)
- image_url = f"...?data={body.data}"
+ data: str = Field(..., min_length=1, max_length=4096)
+ image_url = f"...?data={urllib.parse.quote(body.data)}"
```
+ 2 теста (спецсимволы/Unicode URL-энкодятся, инъекция невозможна; превышение max_length→400). Эталон не ослаблен (12→14 тестов, все зелёные). Пересуд совета:

```text
== ПЕРЕСУД ПОСЛЕ ДОРАБОТКИ ==           deepseek  glm   kimi  большинство
Контракт-паритет                        PASS      PASS  PASS  PASS 3:0
Тесты-живые                             PASS      PASS  PASS  PASS 3:0
Resilience / безопасность               PASS      PASS  PASS  PASS 3:0   ← баг CLOSED 3/3
ОБЩИЙ ВЕРДИКТ                                                  PASS 3:0 (одобрено)
```

💡 **Полный цикл доказан живьём:** произвели → **внутренний луп** (reviewer заворачивает к dev) → PR наверх → **внешний луп** (совет-техлид завернул, поймав то, что ревьюер пропустил) → доработали строго по находкам → совет пересудил → **одобрил единогласно**. Один виток внешней доработки; прошлый баг закрыт 3/3 независимыми судьями.

🎯 **Когда:** любая задача с приёмкой — фича, багфикс, рефактор. Конвейер даёт разделение труда (никто не «сам написал — сам одобрил»), совет — независимую приёмку большинством, что **не доверяет даже собственному ревьюеру слепо**.
⚠️ **Без разделения осей:** либо одна модель пишет-тестирует-ревьюит сама себя (self-preference), либо «совет» просят и произвести, и принять (судьи защищают свою генерацию). Две оси именно потому и разведены.
...
cd ~/workshops/aiweekend
for M in deepseek/deepseek-v4-pro z-ai/glm-5.2 moonshotai/kimi-k2.7-code; do
  opencode run --model "devhands/$M" "$(cat /tmp/defense_prompt.txt)" < /dev/null > "defense_${M##*/}.txt" &
done
wait && echo "три вердикта готовы:" && ls defense_*.txt
```

Поправлено блоков: 31.
