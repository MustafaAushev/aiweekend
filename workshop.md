...
**Плейсхолдеры первичны.** Ядро harness язык-агностик: ты владеешь инвариантами и порогами, а `<ТЕСТ_РАННЕР>`/`<ЛИНТЕР>`/`<ТИП-ЧЕКЕР>`/`<ПАКЕТ-МЕНЕДЖЕР>`/`<МУТАЦИИ>`/`<PROPERTY>` заполняет **совет** под твой стек. Ниже — ОДИН выход совета, профиль под Python; **тот же движок по той же спецификации собирает профиль под Go, TypeScript, Rust** — меняется только строка таблицы. Код руками ты не пишешь.
...
**Совет генерит ВЕСЬ рабочий набор под твой язык — СЛОВАМИ, не скриптом.** Скил `expert-panel` ты собрал в разделе 10. Теперь отдаёшь ему язык-агностик рецепт (инварианты `agents/*.md` + универсальные дев-скилы + плейсхолдеры профиля), и совет материализует конкретику **прямо в каталог проекта** `qrsvc-py/harness/`: `agents/*.md` со стек-профилем, дев-скилы `feature`/`tests`/`review`/`bugfix`, resilience-сниппет, `Makefile`-гейт — и судит собственную генерацию тройкой. **(Это ПЕРВЫЙ язык; второй — тем же вызовом в §16. Всё, что участник использует дальше, — сгенерил совет из универсального harness.)**

> Можно поменять `L1` и `PANEL_SLUG` в первых строках. По умолчанию: основной язык `Python`, папка вердикта `panel/qrsvc-py-harness/verdict.md`.

```bash
export L1=Python
export PANEL_SLUG=qrsvc-py-harness
mkdir -p ~/workshops/aiweekend/qrsvc-py/harness/{agents,skills} && cd ~/workshops/aiweekend
opencode run --model devhands/z-ai/glm-5.2 \
  "прочитай harness/skills/expert-panel.md и действуй по нему как СОВЕТ (3 модели пула). ⚠️ Исходный `harness/` — ТОЛЬКО ДЛЯ ЧТЕНИЯ: НЕ меняй, НЕ перемещай (`mv`), НЕ удаляй ни один файл из `~/workshops/aiweekend/harness/`; ты лишь ЧИТАЕШЬ универсальные файлы и СОЗДАЁШЬ НОВЫЕ в каталоге проекта. Материализуй ПОЛНЫЙ набор под $L1 в ~/workshops/aiweekend/qrsvc-py/harness/. Работай ПО СПИСКУ и НЕ останавливайся, пока КАЖДЫЙ файл не записан на диск. СКИЛЫ — для каждого X из списка [feature, tests, review, bugfix, standard, database]: прочитай harness/skills/X.md И harness/materialization-contract.md, материализуй X в project-grade Python-файл СТРОГО ПО КОНТРАКТУ — все секции обязательны: назначение и когда · входы/выходы (какие файлы читать, какие артефакты создать) · non-negotiables · workflow от спеки до PR по слоям (domain→db→service→api→tests→docs; нет БД → persistence N/A с объяснением) · stack binding БЕЗ плейсхолдеров (реальные uv/ruff/basedpyright/pytest/mutmut/hypothesis/SQLAlchemy async/alembic/httpx/tenacity/pybreaker/pydantic-settings/structlog) · quality gates реальными командами · bad/good примеры ❌/✅ · Definition of Done · stop/escalation rules · audit trail. НЕ конспект: файл короче осмысленной полноты ИЛИ без workflow/gates/examples/DoD/stop-rules ИЛИ с плейсхолдером <...> ИЛИ с учебными лесами («Глоссарий»/«Форма правила»/«Что ломается»/`GATE:`) = FAIL, переделай. Запиши qrsvc-py/harness/skills/X.md. ПРАВИЛА — для каждого X из [resilience, contracts, test-matrix, performance, vcs]: harness/agents/X.md → материализуй по тому же контракту в project-grade Python-правило (non-negotiables + quality gates реальными командами + bad/good + stop-rules, ~80–250 строк) → qrsvc-py/harness/agents/X.md. ГЕЙТ: qrsvc-py/harness/Makefile — цель gate (линт+формат+типы+тесты+покрытие+мутации, стоп на первом красном) + pre-commit hook. Суди тройкой (VERDICT= по каждому файлу), majority, вердикты → panel/$PANEL_SLUG/verdict.md. В КОНЦЕ ОБЯЗАТЕЛЬНО выполни ls qrsvc-py/harness/skills qrsvc-py/harness/agents и убедись, что все 6 скилов + 5 правил + Makefile на месте; чего-то нет — доделай тем же вызовом. Отчитайся списком записанных файлов."
```

**Ожидаем:** в `qrsvc-py/harness/` совет кладёт `agents/*.md` (со стек-профилем), дев-скилы `feature`/`tests`/`review`/`bugfix`, `resilience-<язык>.md`, `Makefile`, pre-commit hook; вердикты — в `panel/<slug>/verdict.md`. Этими скилами §13 строит QR-сервис.
...
**Тестов отдельным шагом здесь нет намеренно:** feature-скил (`harness/skills/feature.md`) сам пишет падающий контракт-тест раньше кода — тесты живут внутри фичи, а не после неё.

**Первая фича — каркас эндпоинта.** 🤖 Промт уже завёрнут в команду, просто запусти. Запускаем из корня: агенту нужны и `qrsvc-py/`, и `harness/`.

```bash
cd ~/workshops/aiweekend
opencode run --model devhands/z-ai/glm-5.2 \
  "ты сейчас в каталоге ~/workshops/aiweekend; и qrsvc-py/, и harness/ — подкаталоги ТЕКУЩЕГО каталога (harness НЕ внутри qrsvc-py!). Создай в qrsvc-py/ uv-проект (uv init --bare; uv add --dev pytest pyyaml hypothesis), затем построй POST /api/v1/qr строго по harness/skills/feature.md; спека qrsvc-py/api/openapi.yaml — источник истины. Тесты пиши ЧАСТЬЮ фичи и раньше кода (как требует feature.md): table-driven на подбор версии + property-тест на hypothesis для нерушимого правила «error_correction в ответе равен запрошенному ИЛИ ответ 422» + e2e-smoke. QR-картинку не генерируй: qr_size — по таблице ёмкостей версий, image_url — заглушка. В конце прогони из qrsvc-py uv run pytest -q --cov=. и назови покрытие; все временные файлы держи внутри qrsvc-py, НЕ в /tmp"
```
...
**Чекпоинт:** прогони `uv run pytest -q --cov=.` сам; найди в логе агента красный прогон до реализации; `grep -r "@given" tests/` находит property-тест.
...
cd ~/workshops/aiweekend
opencode run --model devhands/z-ai/glm-5.2 \
  "ты в ~/workshops/aiweekend (harness/ и qrsvc-py/ — рядом, оба подкаталоги текущего). В qrsvc-py/ сделай тихую зону параметром border строго по harness/skills/feature.md: сначала правка qrsvc-py/api/openapi.yaml (optional integer, minimum 0, maximum 40, default 4; в ответе — required), затем failing contract-тест, затем реализация"
```

**Ожидаем:** дифф начинается со спеки; тест на состав ответа падал до реализации.

**Чекпоинт (ловушка из авторского прогона):** проверь YAML после правок агента — `uv run --with pyyaml python -c "import yaml; yaml.safe_load(open('api/openapi.yaml')); print('YAML OK')"` (в живом примере модель ломала отступы спеки); и проверь `maximum` у `border` — без него `border=100000` даёт гигабайтные PNG.
...
**«Сломай» — единственное доказательство, что тесты живые.** Зелёный набор ничего не стоит, пока не убедишься, что он умеет краснеть. Инжектируй семантический баг — property-тест на «`error_correction` равен запрошенному ИЛИ 422» обязан покраснеть:

Найди, где в ответе собирается `error_correction`:
...
✋ [ручной шаг] В найденном return-словаре ответа захардкодь значение `error_correction` на `"M"`. Если не нашёл в ответе, инжектируй в функцию подбора EC-уровня: property-тест всё равно поймает.
...
**Ожидаем:** после инжекта — КРАСНЫЙ; после отката — зелёный. Не покраснел — тест пустышка (`assert True`-класс): чини тест, а не код.
...
cd ~/workshops/aiweekend
opencode run --model devhands/z-ai/glm-5.2 \
  "ты в ~/workshops/aiweekend (harness/ и qrsvc-py/ рядом). Сделай ревью проекта qrsvc-py/ строго по harness/skills/review.md; SPEC_FILE — qrsvc-py/api/openapi.yaml; вердикт «мержить / не мержить» + находки с привязкой к П-пунктам" < /dev/null > /tmp/review_flash.txt &
opencode run --model devhands/deepseek/deepseek-v4-flash \
  "ты в ~/workshops/aiweekend (harness/ и qrsvc-py/ рядом). Сделай ревью проекта qrsvc-py/ строго по harness/skills/review.md; SPEC_FILE — qrsvc-py/api/openapi.yaml; вердикт «мержить / не мержить» + находки с привязкой к П-пунктам" < /dev/null > /tmp/review_free.txt &
wait
...
**Preflight: каталоги и база паритета.** Первый язык уже должен лежать в `harness/skills/` (часть 2) — **с ним сверяется паритет**, без него сравнивать не с чем. Каталоги `harness/skills` и `panel` должны существовать. opencode подключён к шлюзу Devhands ещё в бутстрапе — отдельный export не нужен. Копипаст:

Запускаем из корня: headless-агенту нужен `harness/` целиком. Первый язык уже должен лежать в `harness/skills/resilience-*.md`: это база, с которой сверяется паритет.

```bash
cd ~/workshops/aiweekend
mkdir -p harness/skills panel
ls harness/skills/resilience-*.md
```
...
**Чекпоинт входных файлов (сделай до сборки).** Всё перечисленное должно лежать на диске после блока генерации harness. Если чего-то нет, внешний репозиторий не нужен: вернись к блоку генерации harness, где панель материализует `agents/*` и `harness/skills/*`; спека копируется из практикума части 2.
...
Артефакты панели берутся из блока генерации harness: `harness/skills/{feature,tests,review}.md`, `agents/{contracts,resilience,performance,test-matrix}.md`.
...
Запускаем из корня: агенту нужны и `qrsvc/`, и `harness/`; из `qrsvc` harness недоступен.
...
cd ~/workshops/aiweekend
opencode run --model devhands/z-ai/glm-5.2 \
  "ты сейчас в каталоге ~/workshops/aiweekend; qrsvc/ и harness/ — подкаталоги ТЕКУЩЕГО каталога (harness НЕ внутри qrsvc!). Построй в qrsvc/ POST /api/v1/qr строго по harness/skills/feature.md; спека qrsvc/api/openapi.yaml источник истины; \
