...
EC_LEVELS = ("L", "M", "Q", "H")
MAX_VERSION = 40
MAX_DATA_BYTES = _BYTE_CAPACITY["L"][-1]  # 2953


def select_version(data_byte_len: int, ec_level: str) -> int | None:
    """Return smallest QR version (1..40) holding data_byte_len bytes for ec_level.

    None if no version fits (i.e. payload exceeds version 40 capacity).
    """
    capacities = _BYTE_CAPACITY[ec_level]
    for version, cap in enumerate(capacities, start=1):
        if data_byte_len <= cap:
            return version
    return None


def capacity(version: int, ec_level: str) -> int:
    return _BYTE_CAPACITY[ec_level][version - 1]
