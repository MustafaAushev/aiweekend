...
> ⚙️ **Развилка части 2.** 🖐 **Канонический путь — РУКАМИ:** копипастишь блоки по ходу чтения и разбираешь каждое правило (так они и откладываются). ⚙️ Опция темпа — агент соберёт весь harness сам, пока идёт разбор: Но если хочешь, чтобы файлы появились на диске фоном — как в шаге 8, — отдай агенту автопрогон: он читает ЭТОТ файл по шагам и выполняет блоки дословно.

**✋ [ручной шаг]:** в блоке ниже подставь свой путь к файлу воркшопа в `WORKSHOP_FILE`.

📝 Сохраняем промт в файл:

```bash
cd ~/workshops/aiweekend
WORKSHOP_FILE="$HOME/workshops/aiweekend/workshop-ai-weekend.md"
...
### 10.6 Анатомия harness — из чего он состоит (чертёж)
...
### 10.7 Python-конкретика: что подставляет совет
...
### 10.11 Self-eval: правила harness
...
### 11.1 Скил: feature — фича как изменение контракта, а не побочный эффект кода
...
#### 11.1.1 🎬 Живой пример: фича мимо контракта против спеки-первой (проверено вживую)
...
### 11.2 Скил: bugfix — Incident-to-Rule Loop: инцидент оставляет после себя правило и eval
...
#### 11.2.1 🎬 Живой пример: закрытый тикет против закрытого класса (проверено вживую)
...
### 11.3 Скил: hotfix — горячая правка с feature flag и путём назад
...
#### 11.3.1 🎬 Живой пример: «решатель» против дисциплины отката (проверено вживую)
...
### 11.4 Скил: refactor — «поведение не изменилось» как доказанная пара прогонов
...
#### 11.4.1 🎬 Живой пример: «поведение не изменилось» против гейта (проверено вживую)
...
### 11.5 Скил: review — ревью как гейт по чеклисту, а не вежливое одобрение
...
#### 11.5.1 🎬 Живой пример: rubber-stamp против чеклиста (проверено вживую)
...
### 11.6 Скил: tests — тесты как руль генерации, mutation против bullshit-тестов
...
#### 11.6.1 🎬 Живой пример: bullshit-тесты против нерушимого правила (проверено вживую)
...
## 12 Практикум: сквозной QR-сервис руками — фича, тесты, покрытие, ревью, гейт
...
### 12.1 Шаг 0 — спека: единственный файл, который кладём руками
...
### 12.2 Шаг 1 — фича №1: каркас эндпоинта feature-скилом (руками — ноль строк)
...
### 12.3 Шаг 2 — фича №2: параметр `border` (кейс из живого примера — теперь твой)
...
### 12.4 Шаг 3 — тесты и покрытие tests-скилом
...
### 12.5 Шаг 4 — ревью двумя моделями: заодно видно, с кем ты работаешь
...
### 12.6 Шаг 5 — гейт на коммит и первый тег вклада
...
### 14.4 Copy-paste: сборка, запуск с равными лимитами, warmup, прогон (§4)
...
#### 14.4.3 JSON — Python (FastAPI) (§4.2)
```bash
...
#### 14.4.4 QR — Go (skip2/go-qrcode) (§4.3)
```bash
...
#### 14.4.6 Запуск с РАВНЫМИ лимитами + warmup + прогон (§4.5)
...
#### 14.4.7 Автопрогон батла агентом (весь стенд — одним промтом) (§4.6)
...
**✋ [ручной шаг]:** в блоке ниже подставь свой путь к этому файлу воркшопа в `WORKSHOP_FILE`.
...
### 14.5 Ожидаемые числа (диапазоны + источник) (§5)
...
#### 14.5.1 JSON (IO/сериализация-bound)
...
### 14.6 Как фиксировать результаты (§6)
...
### 14.7 Честные оговорки (прочитай перед тем, как делать выводы) (§7)
...
### 14.8 Разбор: JSON (IO) vs QR (CPU) — почему разрыв разный (§8)
...
### 14.9 Два захода в этот раздел — что делать и когда (§9)
...
## 19 Батл производительности: два раунда на своих микросервисах
...
### 19.1 Анатомия честного батла (переносимый кирпич)
...
### 19.2 Раунд 1 — JSON (IO-bound): async Python идёт вровень с Go
...
#### 19.2.1 Как фиксировать результаты раунда
...
#### 19.2.2 По шагам (повторяем на своих стендах)
...
#### 19.2.5 Ожидаем (форма результата, не абсолют)
...
#### 19.2.6 Два источника чисел — не путать
...
#### 19.2.7 Честные оговорки этого раунда (обязательно)
...
### 19.3 Разбор раунда 1 + тюнинг по `performance.md`
...
#### 19.3.1 Часть 1 — Разбор: почему разрывы такие
...
#### 19.3.2 Часть 2 — Тюнинг как eval: `performance.md` в деле
...
#### 19.3.3 Что это доказывает (связка с harness)
...
### 19.4 Кульминация — QR (CPU-bound): GIL обнажается, воркеры вытягивают
...
#### 19.4.1 Почему QR обнажает то, что JSON прятал
...
#### 19.4.2 ⚠️ Честный выбор payload (прочитай до замера — это важно)
...
#### 19.4.3 По шагам (демо-payload — тяжёлый, чтобы GIL был виден)
...
#### 19.4.5 Честные оговорки (обязательно перечитай §7)
...
#### 19.4.8 Сшивка двух раундов (финальный аккорд перед веерной оркестрацией ревью и финалом)
...
## 20 После батла: оркестрация по-взрослому, совет как орган контроля и суд
...
### 20.1 Блок: Оркестрация: слои 2–4 (fan-out ревьюеров → synthesizer → защита от каскада)
...
#### 20.1.1 Слой 1 — базовый pipeline (собери прямо сейчас)
...
#### 20.1.2 Слой 2 — Fan-out ревьюеров по категориям на СВОЙ микросервис
...
#### 20.1.3 Слой 3 — Защита от каскада галлюцинаций (cross-check на границе)
...
#### 20.1.4 Слой 4 — Волны и наблюдаемость (опционально)
...
#### 20.1.5 🎬 Живой пример: веер ревью на QR-хэндлере, галлюцинация поймана на границе (проверено вживую)
...
### 20.2 Блок: Экспертный совет по-взрослому: совет как оркестрация, судит итоги батла
...
#### 20.2.1 🎬 Живой пример: совет судит вывод язык-батла (проверено вживую)
...
### 20.3 Блок: Финал: AI SDLC-суд + лидерборд вклада + сертификаты
...
#### 20.3.1 Часть 1 — AI SDLC-суд: комитет судит харнес
...
#### 20.3.2 Часть 2 — Лидерборд вклада: цифры для резюме
...
### 20.4 Self-eval: оркестрация, совет и суд (финальные блоки)
